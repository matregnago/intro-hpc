...
:END:
    ...
    needs:
    - CMake (version 3.12 minimum, 3.17 to compile with the H-Mat support): the build system
    - C and Fortran compilers: GNU compiler suite, Clang, Intel, HPE, or IBM can be used
    ...
    - external libraries: this depends on the configuration, by default
      ...
      - kernels : CBLAS, LAPACKE (with TMG). These are C interfaces to Fortran kernels BLAS and LAPACK.
        There exist several providers that can be used with Chameleon
        (Intel MKL, Netlib, OpenBLAS, BLIS/FLAME, IBM essl, HPE libsci)
    ...
    - rocm: [[https://github.com/ROCm/hip][HIP]] and [[https://github.com/ROCm/hipBLAS][hipBLAS]]
    ...
    cd $HOME/install
    wget https://files.inria.fr/starpu/starpu-1.4.12/starpu-1.4.12.tar.gz
    tar xvf starpu-1.4.12.tar.gz
    cd starpu-1.4.12
...
**** Known issues
     - we need the lapacke interface to tmg routines and symbol like
       ...
       yourself from source or with [[https://spack.io/][Spack]], or with [[https://gitlab.inria.fr/guix-hpc/guix-hpc-non-free][Guix-HPC]].
...
***** MPI
      ...
      *Caution about the compatibility:* OpenMPI should be built with the
      ...
      [[https://www.mpich.org/][MPICH]], [[https://mvapich.cse.ohio-state.edu/overview/][MVAPICH2]] and [[https://pm2.gitlabpages.inria.fr/newmadeleine/][New Madeleine]] can also be used.
***** Nvidia CUDA Toolkit
      ...
      *Caution about the compatibility:* your compiler must be compatible
      with CUDA.
***** HIP
      [[https://rocm.docs.amd.com/projects/HIP/en/latest/install/install.html][HIP]] is a C++ Runtime
...
:END:
    ...
    CMake 3.28 but any version superior to 3.12 should be fine,
    ...
    1. configure:
       #+begin_src
       cmake -S path/to/chameleon -B build -DOPTION1= -DOPTION2= ...
    ...
    2. build:
       #+begin_src
       cmake --build build -j5 --verbose
       # tips: -j[ncores] option to speedup the compilation
       #       --verbose to enable verbose messages
       #       --target install can be added to also install directly after build
       #+end_src
    3. test (optional, required CHAMELEON_ENABLE_TESTING=ON):
       #+begin_src
       ctest --test-dir build --verbose
       # tip: use -R to filter by tests name
       #+end_src
    4. install (optional):
       #+begin_src
       cmake --install build
       ...
       *-DCMAKE_INSTALL_PREFIX* at configure.
       #+begin_example
       cmake -S . -B build -DCMAKE_INSTALL_PREFIX=/home/jdoe/install/chameleon
       #+end_example
...
**** Configuration options
     ...
     invoking ~cmake -B build -DOPTION1= -DOPTION2= ...~
     #+begin_src
     cmake -S . -B build -DCMAKE_BUILD_TYPE=Debug \
                         -DCMAKE_INSTALL_PREFIX=/home/jdoe/install/chameleon \
...
***** Chameleon specific options
      ...
      * *CHAMELEON_USE_HIP=ON|OFF* (default OFF): to link with HIP
      ...
      * *CHAMELEON_VERBOSE* (default OFF): Enable debug information from Chameleon
        (Use CHAMELEON_SILENT to dynamically turn off)
      * *CHAMELEON_BATCH_SIZE* (default 10): Maximum size for the batched kernels
...
**** Dependencies detection
     ...
     1) using [[https://cmake.org/cmake/help/latest/variable/CMAKE_PREFIX_PATH.html#variable:CMAKE_PREFIX_PATH][CMAKE_PREFIX_PATH]] for libraries that provide some CMake config
        files containing targets definitions (e.g. fooConfig.cmake).
     2) using [[https://www.freedesktop.org/wiki/Software/pkg-config/][pkg-config]] for libraries that provide .pc files
        - update your *PKG_CONFIG_PATH* to the paths where to find .pc
          files of installed external libraries like hwloc, starpu, some
          blas/lapack, etc
     3) detection of dependencies through environment variables:
        ...
        - INCLUDE (or CPATH, or C_INCLUDE_PATH)should contain the list
          ...
          #+end_src
     4) detection with user's given paths:
        - you can specify the path at cmake configure by invoking
          #+begin_example
          cmake -B build -DLIB_DIR=path/to/your/lib
          #+end_example
          where LIB stands for the name of the lib to look for, /e.g./
          #+begin_src
          cmake -B build -DQUARK_DIR=path/to/quarkdir -DCBLAS_DIR= ...
          #+end_src
          ...
          #+end_src
     5) detection with custom environment variables: all variables like
     ...
     Explore this [[https://solverstack.gitlabpages.inria.fr/pages/faq.html#faq4][FAQ]] to see how to give specific BLAS/LAPACK libraries to CMake.
...
**** Installing Chameleon with Guix
     ...
     - chameleon (default) : with starpu - with openmpi - with OpenBlas
     - chameleon-nompi : idem chameleon but without openmpi
     - chameleon-nmad : idem chameleon but nmad replaces openmpi
     - chameleon-aocl : idem chameleon but AMD FLAME/BLIS replaces OpenBlas
     - chameleon-mkl : idem chameleon but Intel MKL sequential replaces OpenBlas
     - chameleon-mkl-mt : idem chameleon but Intel MKL multithreaded replaces OpenBlas
     - chameleon-mkl-nompi : idem chameleon-mkl but without openmpi
     - chameleon-mkl-mt-nompi : idem chameleon-mkl-mt but without openmpi
     - chameleon-cuda : with starpu - with cuda - with OpenBlas - with openmpi
     - chameleon-cuda-nompi : idem chameleon-cuda but without openmpi
     - chameleon-cuda-mkl : idem chameleon-cuda Intel MKL sequential replaces OpenBlas
     - chameleon-cuda-mkl-nompi : idem chameleon-cuda-mkl but without openmpi
     - chameleon-cuda-mkl-mt : idem chameleon-cuda but Intel MKL multithreaded replaces OpenBlas
     - chameleon-cuda-mkl-mt-nompi : idem chameleon-cuda-mkl-mt but without openmpi
     - chameleon-hip : with starpu - with hip - with OpenBlas - with openmpi
     - chameleon-hip-nompi : idem chameleon-hip but without openmpi
     - chameleon-hip-mkl-mt : idem chameleon-hip but Intel MKL multithreaded replaces OpenBlas
     - chameleon-hip-mkl-mt-nompi : idem chameleon-hip-mkl-mt but without openmpi
     - chameleon-simgrid : with starpu - with openmpi - with simgrid
     - chameleon-simgrid-nosmpi : with starpu - without openmpi - with simgrid
     - chameleon-openmp : with openmp - without openmpi
     - chameleon-parsec : with parsec - without openmpi
     - chameleon-quark : with quark - without openmpi
...
**** Installing Chameleon with Spack
     ...
     Notice also that default mpi is OpenMPI and default blas/lapack is
     Openblas. This can be changed by adding some [[https://spack.readthedocs.io/en/latest/basic_usage.html#constraining-virtual-packages][constraints on virtual packages]].

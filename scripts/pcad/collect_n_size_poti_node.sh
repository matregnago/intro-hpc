...
# (`sbatch -p poti --gres=gpu:1 --cpus-per-task=20 --wrap 'sleep infinity'`).
# Equivalente interativo do slurm/gpu_tile_poti.slurm mas para o EXPERIMENTO
# E2b (escalonamento vs tamanho do problema N, no pico de tile de E1).
#
# DoE: final/doe_gpu_n_size_poti.csv (320 runs: 2 alg FP64 x 4 runtime/scheduler
# x 8 Ns x 5 reps, todos com b=500 -- o pico de tile FP64 achado em E1 / job
# 798331). Sem traces (so time/gflops) -- TRACE=0. Modelos dmda/dmdas do StarPU
# sao CALIBRADOS antes das medidas (CALIB_PASSES=2, modelo isolado em
# $RESULTS_DIR/.starpu, congelado p/ as runs cronometradas com STARPU_CALIBRATE=0).
#
# Normalmente disparado pelo front-end (scripts/pcad/collect_n_size_poti_pcad.sh),
# mas roda direto se voce ja esta no no:
#   bash scripts/pcad/collect_n_size_poti_node.sh
#
# Env: SUBMIT_DIR (repo no home, default ~/intro_hpc), SCRATCH, JOBTAG,
#      THREADS (override, default 19), GPUS (1).
...
case "$HOST" in
    ...
    *)    echo "!! no desconhecido: $HOST (esperado poti)" >&2; exit 1 ;;
esac
DOE_CSV="final/doe_gpu_n_size_poti.csv"
...
# DoE de N-size para a poti (320 runs: FP64, b=500, 8 Ns x 2 alg x 4 sched x 5 reps).

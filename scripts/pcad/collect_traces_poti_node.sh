...
# Equivalente interativo do slurm/gpu_traces_fair.slurm: rsync do repo ->
# $SCRATCH, roda as duas fases (starpu/parsec) do gpu_doe_sweep.sh la (I/O
# rapido), e copia os resultados de volta pro $HOME via trap EXIT (copyback).
#
# DoE: final/doe_gpu_traces_poti_500.csv (8 runs: 2 algoritmos FP64 x 4
# runtime/scheduler x 1 rep, N=40000, b=500 -- pico de E1). Captura rastro
# completo (TRACE=1 + TRACE_FULL=1 + TRACE_STATS=1):
...
#
# Normalmente disparado pelo front-end (scripts/pcad/collect_traces_poti_pcad.sh),
# mas roda direto se voce ja esta no no:
#   bash scripts/pcad/collect_traces_poti_node.sh
#
...
DOE_CSV="final/doe_gpu_traces_poti_500.csv"
...
# DoE de traces para a poti (8 runs: FP64, N=40000, b=500, 1 rep).
